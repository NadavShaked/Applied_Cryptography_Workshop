...
import secrets
import math
from z_prime_field import ZPrimeField
...
def bytes_needed(number: int) -> int:
    ...
    byte_count = math.ceil(bit_length / 8)
    power_of_two_bytes = 2**math.ceil(math.log2(byte_count))

    return power_of_two_bytes


def galois_field_element_to_bytes(element: int, num_bytes: int) -> bytes:
    """
...
def get_blocks_authenticators_by_file_path(
    ...
    with open(file_path, "rb") as f:
        ...
        while True:
            ...
            sigma_i_in_bytes = galois_field_element_to_bytes(sigma_i, mac_size)
...
def secure_random_sample(maxIndex: int, number_of_indices: int) -> list[int]:
    ...
    indices = list(range(maxIndex))

    # Shuffle the list using cryptographic randomness
    for i in range(len(indices) - 1, 0, -1):
        j = secrets.randbelow(i + 1)  # Get a secure random index
